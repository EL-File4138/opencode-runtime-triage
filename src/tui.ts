...
type Preset = { label: string; provider: string; model: string; variant?: string; description?: string }
...
export default Plugin.define({
  ...
  setup(ctx) {
    ...
    const locations = new Map<string, { directory: string }>()
    const presets = Array.isArray(ctx.options.presets) ? ctx.options.presets.filter(isPreset) : []
    const location = () => ctx.location ?? ctx.data.location.default()
    const toast = (message: string, variant: "success" | "error" | "warning" = "success") =>
      ctx.ui.toast.show({ title: "Runtime triage", message, variant })
    const apply = async (at: { directory: string }, changes: { agent: string; model: ModelRef | null }[]) => {
      ...
      if (sessionID) await ctx.data.session.sync(sessionID)
      toast(`Updated ${changes.length} agent${changes.length === 1 ? "" : "s"}`)
    }
    const guard = (run: () => Promise<void>) => async () => {
      try { await run() } catch (error) { toast(String(error), "error") }
    }
    const selectModel = guard(async () => {
      ...
      const models = new Map<string, { title: string; value: string; category: string; description: string }>()
      ...
      for (const preset of presets) {
        ...
        if (option) models.set(key, { ...option, title: preset.label, description: preset.description ?? key })
      }
    ...
    const selectProvider = guard(async () => {
      ...
      if (!changes.length) { toast("No matching models and variants", "warning"); return }
    ...
    const timer = setInterval(() => {
      for (const at of locations.values()) void rpc.heartbeat({ owner }, { location: at }).catch((error) => toast(String(error), "error"))
    }, 20_000)
    return async () => {
      clearInterval(timer)
