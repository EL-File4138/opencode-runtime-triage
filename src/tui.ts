...
type RuntimeTriageOptions = {
  presets?: ModelPreset[]
}

const isPreset = (value: unknown): value is ModelPreset => {
...
const tui: TuiPlugin = async (api, rawOptions) => {
  ...
  const applyProviderOverride = async (
    ...
    const targetModels = api.state.provider.find(
    ...
    const matches = matchingProviderOverrides(
      agentEntries(),
      runtimeModels,
      sourceProvider,
      targetProvider,
      new Set(Object.keys(targetModels)),
    )
    if (matches.length === 0) {
      api.ui.toast({
        variant: "warning",
    ...
    const previous = new Map(
      matches.map(({ agent }) => [agent, runtimeModels?.get(agent)]),
    )
  ...
  const selectTargetProvider = (sourceProvider: string) => {
    const runtimeModels = getRuntimeModels(api.state.path.directory)
    const sourceModels = agentEntries().flatMap(
      ([agent, config]) => {
    ...
    api.ui.dialog.replace(() =>
      api.ui.DialogSelect({
        ...
        options: api.state.provider
          ...
          .map((provider) => {
            const matchCount = sourceModels.filter(
        ...
        onSelect: (option) =>
          void applyProviderOverride(sourceProvider, option.value),
      }),
    )
  }
  ...
  const selectSourceProvider = () => {
    ...
    for (const [agent, config] of agentEntries()) {
      ...
      providerCounts.set(
        current.provider,
        (providerCounts.get(current.provider) ?? 0) + 1,
      )
    }
    ...
    api.ui.dialog.replace(() =>
      api.ui.DialogSelect({
        ...
        options: [...providerCounts]
          .map(([provider, count]) => ({
            title:
              api.state.provider.find((item) => item.id === provider)?.name ??
              provider,
            value: provider,
