import { OpenCode, type OpenCodeClient, type SessionFormCreateInput } from "@opencode/client"
import { Service } from "@opencode/client/service"
import { setTimeout as delay } from "node:timers/promises"
import { Runtime } from "./rpc.js"

export async function formClient(instance: string, directory: string): Promise<OpenCodeClient> {
  const endpoint = await Service.discover()
  if (!endpoint) throw new Error("Desktop forms require the managed OpenCode service. Use command arguments on a standalone server.")
  const client = OpenCode.make({ baseUrl: endpoint.url, headers: Service.headers(endpoint) })
  const identity = await client.rpc(Runtime).instance({}, { location: { directory } })
  if (identity !== instance) throw new Error("The discovered service is not this plugin's server. Use command arguments on this server.")
  return client
}

export class RuntimeForms {
  private active = new Map<string, { controller: AbortController; done: Promise<void> }>()
  constructor(private readonly connect: () => Promise<OpenCodeClient>) {}

  async open(input: SessionFormCreateInput, apply: (answer: Record<string, unknown>) => Promise<void>) {
    if (this.active.has(input.sessionID)) throw new Error("A runtime triage form is already open in this session")
      ...
      task.done = (async () => {
        try {
          while (!controller.signal.aborted) {
            ...
            if (detail.state.status === "cancelled") return
          ...
          if (controller.signal.aborted) await client.session.form.cancel(ref).catch(console.error)
